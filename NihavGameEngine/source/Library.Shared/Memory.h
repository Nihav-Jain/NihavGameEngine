...
#define ENGINE_ALLOC(x, id, tag) Library::HeapManager::GetHeapManager().AllocateBlock(heapId, x, __FILE__, __LINE__, tag, std::chrono::milliseconds(0))
#define ENGINE_ALLOC_ALIGNED(x, id, tag, alignment) Library::HeapManager::GetHeapManager().AllocateBlock(heapId, x, __FILE__, __LINE__, tag, std::chrono::milliseconds(0), alignment)

#define ENGINE_FREE(ptr) Library::HeapManager::GetHeapManager().FreeBlock(ptr)
#define ENGINE_DELETE(ptr) delete(ptr, 1, __FILE__, __LINE__, "Default", std::chrono::milliseconds(0), 4)
